...
function mergeMeshes(meshes: Mesh[], cloneGeometry: THREE.BufferGeometry | true = true)
{
...
function* runPathTracingLoop()
{
	...
	const bounces = Math.floor(params.pathTracing.bounces);
	const skyIntensity = params.environment.skyIntensity;
	...
	const rayStack = new Array(bounces).fill(undefined).map(() => new THREE.Ray());
	...
	function pathTrace(ray, targetColor)
	{
		...
		for (let i = 0; i < bounces; i++)
		{
			...
			let hit: THREE.Intersection<THREE.Object3D> & {
				normal: THREE.Vector3
				geometryNormal: THREE.Vector3
				material: Material
			} | null = null;
			...
			hit = raycaster.intersectObjects(objects, true)[0] as any;
			...
			if (hit)
			{
				...
				} else
				{
					...
					const { emissive, emissiveIntensity } = material as THREE.MeshStandardMaterial;
...
function render()
{
	...
	if (models[params.model])
	{
		...
		material.emissiveIntensity = params.material.emissiveIntensity;
		material.ior = params.material.ior;
		material.metalness = params.material.metalness;
		material.transmission = params.material.transmission;
		...
		material.roughness = Math.pow(params.material.roughness, 2.0);
